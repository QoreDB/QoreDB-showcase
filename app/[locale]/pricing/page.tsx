...
export default function PricingPage() {
	...
	const [proStripePrice, setProStripePrice] = useState<string | null>(null);
	...
	useEffect(() => {
		...
		const loadPrice = async () => {
			try {
				...
				const data = (await response.json()) as { formattedPrice?: string };
				if (!cancelled && data.formattedPrice) {
					setProStripePrice(data.formattedPrice);
				}
			} catch (error) {
				console.error("Failed to load Stripe price", error);
			}
		};
					...
					<div className="mt-12 grid grid-cols-1 lg:grid-cols-3 gap-6">
						...
						>
							<PlanCard
								...
								price={proStripePrice ?? t("pricing_page.pro.price")}
