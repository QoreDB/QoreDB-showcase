...
QoreDB is a desktop database client for developers. From a single application, you can connect to PostgreSQL, MySQL, MongoDB, SQLite, SQL Server, Redis, DuckDB, CockroachDB and more, browse schemas, write queries, edit data, and run notebooks.
...
**Connect to anything.** PostgreSQL (and Supabase, Neon, TimescaleDB), MySQL/MariaDB, SQL Server, SQLite, MongoDB, Redis, DuckDB, CockroachDB. Driver auto-detection from the connection string: paste a DSN and QoreDB picks the right driver automatically. SSH tunneling with proxy jump support is built-in.
