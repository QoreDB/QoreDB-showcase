...
When such a pattern is detected against a **production** connection, a confirmation dialog blocks the run and asks you to type the connection or table name to proceed. The exact threshold depends on the connection's environment tag (development, staging, production) and the safety settings.
