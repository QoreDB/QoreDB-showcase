...
Toggle the sandbox from the title bar (Flask icon) or with <KeyboardShortcut shortcut="Cmd+Shift+S" /> (⌘⇧S on macOS, Ctrl+Shift+S on Windows/Linux). When on, the connection's UI shows a colored sandbox indicator (blue for development, orange for staging, red for production) and a count of pending changes.
...
- **No DDL diff in the sandbox**: the sandbox captures data changes, not schema changes. To compare the *contents* of two tables side by side, see [Data diff](/docs/diff-and-migrations/data-diff). To compare schemas (DDL) between databases and generate the migration, use [Schema diff](/docs/diff-and-migrations/schema-diff), a Pro feature.
