...
Toggle the sandbox from the title bar (Flask icon) or with <KeyboardShortcut shortcut="Cmd+Shift+S" /> (⌘⇧S on macOS, Ctrl+Shift+S on Windows/Linux). The sandbox is **per session**, meaning per active connection: turning it on for one connection doesn't affect the others.
